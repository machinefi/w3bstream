...
func newInstanceStartCmd(client client.Client) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := Start(client, args[0]); err != nil {
				return errors.Wrap(err, fmt.Sprintf("problem start instance %+v", args))
			}
			cmd.Printf("instance %s started successfully\n", args[0])
			return nil
		},
	}
}

func Start(client client.Client, insID string) error {
	url := getInstanceCmdUrl(client.Config().Endpoint, insID, "START")
