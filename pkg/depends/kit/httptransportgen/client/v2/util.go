...
func MayPrefixDeprecated(desc string, deprecated bool) []string {
	...
	if deprecated {
		comments = append(comments, "@deprecated")
	}
	if desc != "" {
...
func GetEnumOptions(schema *oas.Schema) enumgen.Options {
	labels, ok := schema.Extensions[openapi.XEnumLabels]
	if !ok {
		return nil
	}
...
func AddTag(tags map[string][]string, k string, flags ...string) {
	tags[k] = append(tags[k], flags...)
}

func AddKvTag(tags map[string][]string, k, v string, required bool) {
	AddTag(tags, k, v)
	if !required && !strings.Contains(v, "omitempty") {
		AddTag(tags, k, "omitempty")
	}
}

func LastPackages(path string, level int) []string {
	paths := make([]string, 0)
	...
	sort.Slice(paths, func(i, j int) bool { return true })
