...
func (d *dwarfInfo) getLineReader() *dwarf.LineReader {
	...
	if err != nil {
		// logr.Errorf("[wasmer][debug] getLineReader fail to do d.Reader().Next(), err: %v", err)
	}

	lineReader, err := d.data.LineReader(cu)
	if err != nil {
		// logr.Errorf("[wasmer][debug] getLineReader fail to do d.LineReader(), err: %v", err)
	}
	...
	for i < len(data) {
		...
		if true /* debug mode */ {
			if len(sectionName) > 0 {
				// logr.Debugf("[wasmer][debug] loadCustomSections %v(%X) %v: data start at 0x%X, data size 0x%X", secTypeNames[sectionType], sectionType, sectionName, i, sectionDataSize)
			} else {
				// logr.Debugf("[wasmer][debug] loadCustomSections %v(%X): data start at 0x%X, data size 0x%X", secTypeNames[sectionType], sectionType, i, sectionDataSize)
			}
		}
