...
func (d *DwarfInfo) GetLineReader() *dwarf.LineReader {
	...
	if err != nil {
		log.Printf("[wasmtime][debug] getLineReader fail to do d.Reader().Next(), err: %v", err)
	}

	lineReader, err := d.data.LineReader(cu)
	if err != nil {
		log.Printf("[wasmtime][debug] getLineReader fail to do d.LineReader(), err: %v", err)
	}
...
func ParseDwarf(data []byte) *DwarfInfo {
	...
	if err != nil {
		log.Printf("[wasmtime][debug] dwarf.New failed: %v", err)
		return nil
	}
	...
	for i < len(data) {
		...
		if true /* debug mode */ {
			if len(sectionName) > 0 {
				log.Printf("[wasmtime][debug] LoadCustomSections %v(%X) %v: data start at 0x%X, data size 0x%X", secTypeNames[sectionType], sectionType, sectionName, i, sectionDataSize)
			} else {
				log.Printf("[wasmtime][debug] LoadCustomSections %v(%X): data start at 0x%X, data size 0x%X", secTypeNames[sectionType], sectionType, i, sectionDataSize)
			}
		}
