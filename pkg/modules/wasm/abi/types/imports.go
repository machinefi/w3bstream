...
type ImportsContext interface {
	Entry(contextID string) bool
	Leave(contextID string) bool
	ContextID() string
}
...
func NewDefaultImports() ImportsHandler {
	return struct {
		...
		ImportsContext:        &DefaultImportsContext{},
...
func NewDefaultImportsContext() *DefaultImportsContext {
	v := atomic.NewString("")
	v.Store("")
	return &DefaultImportsContext{contextID: v}
}

type DefaultImportsContext struct {
	contextID *atomic.String
}

func (c *DefaultImportsContext) Entry(cid string) bool {
	return c.contextID.CompareAndSwap("", cid)
}
