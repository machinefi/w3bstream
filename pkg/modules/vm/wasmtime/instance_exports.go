...
func (ef *ExportFuncs) SetSQLDB(addr, size int32) int32 {
	...
	_, err = ef.db.WithDefaultSchema().ExecContext(context.Background(), prestate, params...)
	if err != nil {
		ef.log.Error(err)
		return wasm.ResultStatusCode_Failed
	}
...
func (ef *ExportFuncs) GetSQLDB(addr, size int32, vmAddrPtr, vmSizePtr int32) int32 {
	...
	rows, err := ef.db.WithDefaultSchema().QueryContext(context.Background(), prestate, params...)
	if err != nil {
		ef.log.Error(err)
		return wasm.ResultStatusCode_Failed
	}
