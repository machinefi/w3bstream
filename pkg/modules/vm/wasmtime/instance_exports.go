...
func (ef *ExportFuncs) LinkABI(impt Import) error {
	for name, ff := range map[string]interface{}{
		...
		"ws_send_tx_with_private_key": ef.SendTXWithPrivateKey,
...
func (ef *ExportFuncs) loggingAndPersDB(logLevel conflog.Level, logSrc, msg string) {
	if len(logSrc) == 0 {
		logSrc = efSrc
	}
...
func (ef *ExportFuncs) Log(logLevel, ptr, size int32) int32 {
	...
	if err != nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, codeSrc, err.Error())
		return wasm.ResultStatusCode_Failed
	}
	ef.loggingAndPersDB(conflog.Level(logLevel), codeSrc, string(buf))
...
func (ef *ExportFuncs) Abort(msgPtr int32, fileNamePtr int32, line int32, col int32) {
	...
	if err != nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, efSrc, errors.Wrap(err, "fail to decode arguments in env.abort").Error())
		return
	}
	...
	ef.loggingAndPersDB(conflog.ErrorLevel, efSrc, errors.Errorf("abort: %s at %s:%d:%d", msg, fileName, line, col).Error())
}
...
func (ef *ExportFuncs) Trace(msgPtr int32, _ int32, arr ...float64) {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, efSrc, fmt.Sprintf("trace: %s%s", msg, str))
}
...
func (ef *ExportFuncs) GetDB(kAddr, kSize int32, vmAddrPtr, vmSizePtr int32) int32 {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, efSrc, fmt.Sprintf("host.GetDB %s:%s", string(key), strconv.Quote(string(val))))
...
func (ef *ExportFuncs) SetDB(kAddr, kSize, vAddr, vSize int32) int32 {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, efSrc, fmt.Sprintf("host.SetDB %s:%s", string(key), strconv.Quote(string(val))))
...
func (ef *ExportFuncs) SendTXWithPrivateKey(chainID int32, offset, size, vmAddrPtr, vmSizePtr int32) int32 {
	if ef.cl == nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, efSrc, errors.New("eth client doesn't exist").Error())
		return wasm.ResultStatusCode_Failed
	}
	...
	ret := gjson.Parse(string(buf))
	txHash, err := ef.cl.SendTXWithPrivateKey(uint32(chainID), ret.Get("to").String(), ret.Get("value").String(), ret.Get("data").String(), ret.Get("privateKey").String())
	if err != nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, efSrc, err.Error())
		return wasm.ResultStatusCode_Failed
	}
...
func (ef *ExportFuncs) SendMqttMsg(topicAddr, topicSize, msgAddr, msgSize int32) int32 {
	if ef.mq == nil || ef.mq.Client == nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, efSrc, errors.New("mq client doesn't exist").Error())
		return wasm.ResultStatusCode_Failed
	}
