...
func (ef *ExportFuncs) loggingAndPersDB(logLevel conflog.Level, msg string) {
	switch logLevel {
	...
	job.Dispatch(ef.ctx, job.NewWasmLogTask(ef.ctx, logLevel.String(), msg))
}

func (ef *ExportFuncs) Log(logLevel, ptr, size int32) int32 {
	buf, err := ef.rt.Read(ptr, size)
	if err != nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, err.Error())
		return wasm.ResultStatusCode_Failed
	}
	ef.loggingAndPersDB(conflog.Level(logLevel), string(buf))
...
func (ef *ExportFuncs) Abort(msgPtr int32, fileNamePtr int32, line int32, col int32) {
	...
	if err != nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, errors.Wrap(err, "fail to decode arguments in env.abort").Error())
		return
	}
	...
	ef.loggingAndPersDB(conflog.ErrorLevel, errors.Errorf("abort: %s at %s:%d:%d", msg, fileName, line, col).Error())
}
...
func (ef *ExportFuncs) Trace(msgPtr int32, _ int32, arr ...float64) {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, fmt.Sprintf("trace: %s%s", msg, str))
}
...
func (ef *ExportFuncs) GetDB(kAddr, kSize int32, vmAddrPtr, vmSizePtr int32) int32 {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, fmt.Sprintf("host.GetDB %s:%s", string(key), strconv.Quote(string(val))))
...
func (ef *ExportFuncs) SetDB(kAddr, kSize, vAddr, vSize int32) int32 {
	...
	ef.loggingAndPersDB(conflog.InfoLevel, fmt.Sprintf("host.SetDB %s:%s", string(key), strconv.Quote(string(val))))
...
func (ef *ExportFuncs) SendTX(chainID int32, offset, size, vmAddrPtr, vmSizePtr int32) int32 {
	if ef.cl == nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, errors.New("eth client doesn't exist").Error())
		return wasm.ResultStatusCode_Failed
	}
...
func (ef *ExportFuncs) SendMqttMsg(topicAddr, topicSize, msgAddr, msgSize int32) int32 {
	if ef.mq == nil || ef.mq.Client == nil {
		ef.loggingAndPersDB(conflog.ErrorLevel, errors.New("mq client doesn't exist").Error())
		return wasm.ResultStatusCode_Failed
	}
